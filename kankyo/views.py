...
def home(request):
	...
	return render(request,'sarcasmbase.html', context)
...
def random(request):
	cur_user = Team.objects.get(user=request.user)
	cur_level = cur_user.current_level	
	image = "/static/"+cur_user.team_logo+".png"
	return render(request,'users/randomshit.html', {"image":image})


def updateBonusLevel():
	teams = Team.objects.all()
	for team in teams:
		if team.bonus_level_id != 2 and team.bonus_attempted == 0:
			team.bonus_level_id = 1
			team.bonus_attempted = 0
			team.save()

def checkBonusLevel():
	teams = Team.objects.all()
	for team in teams:
		if team.bonus_level_id != 1:
			print(team.leader_roll_number)


def checklevel21():
	teams = Team.objects.all()
	for team in teams:
		if team.current_level.level_id == 22:
			print(team.team_name, team.latest_question_date)